...
orig_epochs = loadtxt('combined_1_2_3_4_5_800_1800.csv', delimiter=',', skiprows=1)
print(orig_epochs.shape)

# scale the data with robust scaler
my_epochs = rScaler.fit_transform(orig_epochs[0:1490, 0:1920])  # noise
...
input = train_data[:, 0:1920]
testinput = X_test_tmp[:, 0:1920]

#=====================================

# Model configuration

input = input.reshape(len(input), 30, 64)
input = input.transpose(0, 2, 1)
print (input.shape)

testinput = testinput.reshape(len(testinput), 30, 64)
testinput = testinput.transpose(0, 2, 1)
print (testinput.shape)

# Create the model  ################## BEST ONE - SO FAR - 5 participants #######################
model=Sequential()
model.add(Conv1D(filters=60, kernel_size=5, kernel_regularizer=L2(0.001), bias_regularizer=L2(0.001), activity_regularizer = L2(0.001), kernel_constraint=min_max_norm(min_value=-2, max_value=2), padding='valid', activation='relu', strides=1, input_shape=(64, 30)))
model.add(AveragePooling1D(pool_size=3))
model.add(Conv1D(filters=60, kernel_size=5, kernel_regularizer=L2(0.001), bias_regularizer=L2(0.001), activity_regularizer = L2(0.001), kernel_constraint=min_max_norm(min_value=-2, max_value=2), padding='valid', activation='relu', strides=1))
model.add(AveragePooling1D(pool_size=3))
model.add(Conv1D(filters=20, kernel_size=5, kernel_regularizer=L2(0.001), bias_regularizer=L2(0.001), activity_regularizer = L2(0.001), kernel_constraint=min_max_norm(min_value=-2, max_value=2), padding='valid', activation='relu', strides=1))
model.add(GlobalMaxPooling1D())
model.add(Dense(40, kernel_regularizer=L2(0.001), bias_regularizer=L2(0.001), activity_regularizer = L2(0.001), kernel_constraint=min_max_norm(min_value=-2, max_value=2), activation='relu'))
model.add(Dense(3, activation='softmax'))

model.summary()

# Compile the model   
adam = Adam(learning_rate=0.000085)
